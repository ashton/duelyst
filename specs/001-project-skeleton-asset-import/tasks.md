...
- [X] T038 [P] Run the full [quickstart.md](./quickstart.md) validation (all 9 scenarios, including Path A / scenario 9 for SC-001/SC-006) and record results — **8/9 fully green, 1 partial**: (1) reproducible byte-identical ✓, (2) no plist/xml under `assets/` ✓, (3) `npm run verify` clean (0 issues, 160 files) ✓, (4) fail-loud on invalid `external/duelyst/` ✓, (5) `dotnet test` 17/17 green ✓, (6) `npm test` 15/15 green ✓, (7) client shows sprite within 10s — **partial**: resolve/parse verified correct, real on-screen pixels unconfirmed (no display in this sandbox, user declined installing Xvfb), (8) `dotnet build Duelyst.sln` clean ✓, (9) Path A fresh-clone timing ✓ — after committing (root commit `99f6a38`), `git clone` (local) + `git lfs pull` + `dotnet build` completed in ~6s total, well under the 15-min bound (SC-001/SC-006); `dotnet run --project src/Duelyst.Client` from the fresh clone hit the same known no-X11-display limitation as (7), confirming the fresh-clone path itself (asset resolution, texture load init) is sound up to the windowing layer.
